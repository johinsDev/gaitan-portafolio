...
export function FeatureSectionLayout({ data }: Props) {
  ...
  return <section className={cn("py-8 md:py-12 flex items-center gap-4", {
    'flex-col md:flex-row-reverse': position === 'left',
    'flex-col-reverse md:flex-row': position === 'right',
  })}>
    <div className={cn("flex-1 flex flex-col items-center text-center", {
    ...
    {!!imageUrl && <div className="flex-shrink-0 flex justify-end">
      <Image src={imageUrl} width={444} height={300} alt={data?.image?.alt ?? ''} className="object-cover object-top aspect-video" />
    </div>}
  </section>
